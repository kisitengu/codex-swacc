...
function createDashboardServer({
  ...
  async function handleApi(request, response, url) {
    ...
    if (url.pathname === "/api/rotate" && request.method === "POST") {
      ...
      const sqliteOutput = store.kind === "sqlite";
      const outputPath = sqliteOutput
        ? path.join(tempDir, "accounts.rotated.txt")
        : path.resolve(
          payload.outputPath
            ? String(payload.outputPath)
            : defaultOutputPath(store.filePath),
        );
      ...
      const job = {
        ...
        output: sqliteOutput ? "SQLite database" : path.basename(outputPath),
      ...
      rotate(rotationArgs, {
        ...
        Object.assign(job, {
          ...
          result: {
            ...
            output: sqliteOutput ? store.filePath : outputPath,
          },
...
<head>
  ...
  <style>
    ...
    input { width: 100%; box-sizing: border-box; background: #111827; color: #f9fafb; border: 1px solid #4b5563; border-radius: 7px; padding: 9px 10px; }
...
(() => {
  ...
  $("#rotate").onclick = async () => {
    if (!confirm("Generate new passwords for all accounts? The output will be written next to the source list.")) return;
    try {
      const data = await request("/api/rotate", { method: "POST", body: JSON.stringify({}) });
      status("Rotation started. Keep this dashboard open and watch the terminal for browser verification.");
