...
function formatCodexAppServerError(error) {
...
function runCodexAuthLogin(codexHomeForLogin, {
  ...
  return new Promise((resolve, reject) => {
    ...
    const completeIfReady = () => {
      if (browserStarted && loginCompleted && authValidated) {
        stop();
      }
    };
    ...
    const handleMessage = (message) => {
      ...
      if (message.id === 3) {
        if (message.error) {
          stop(new Error(
            `Codex auth validation failed: ${formatCodexAppServerError(message.error)}`,
          ));
          return;
        }
        authValidated = true;
        onStep("Codex auth was validated against the quota API.");
        completeIfReady();
        return;
      }
...
function runCodexAuthRefresh(codexHomeForRefresh, {
  ...
  return new Promise((resolve, reject) => {
    const child = spawnCodexProcess([
      ...
      env: {
        ...process.env,
        CODEX_HOME: codexHomeForRefresh,
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
    let stdoutBuffer = "";
    ...
    const handleMessage = (message) => {
      ...
      if (message.error) {
        const action = message.id === 2 ? "token refresh" : "auth validation";
        stop(new Error(
          `Codex ${action} failed: ${formatCodexAppServerError(message.error)}`,
        ));
        return;
      }
      if (message.id === 2) {
        onStep("Codex refreshed the saved ChatGPT session.");
        child.stdin.write(`${JSON.stringify({
          id: 3,
          method: "account/rateLimits/read",
        })}\n`);
    ...
    const timer = setTimeout(() => {
      stop(new Error("Timed out waiting for Codex token refresh."));
    }, timeoutMs);
    ...
    child.on("close", (code, signal) => {
      ...
      const reason = signal ? `signal ${signal}` : `exit code ${code}`;
      const detail = stderr.trim() ? `: ${stderr.trim()}` : "";
      resultError = new Error(`Codex app-server exited during token refresh (${reason})${detail}`);
      settle();
    });
...
async function refreshExistingAuthProfile(profile, {
  expectedEmail,
  directory = profilesDir(),
  timeoutMs = PAGE_TIMEOUT_MS,
  spawnCodexProcess,
  ...
  const source = profilePath(profile.name, directory);
  const { contents: originalContents } = readAuthFile(source);
  const tempHome = createTempCodexHome();
  try {
    writePrivateAtomic(path.join(tempHome, "auth.json"), originalContents);
    onStep(`Refreshing Codex auth profile ${profile.fileName}.`);
    await runCodexAuthRefresh(tempHome, {
      timeoutMs,
      spawnCodexProcess,
      onStep,
    });
    const { contents: refreshedContents } = readAuthFile(
      path.join(tempHome, "auth.json"),
    );
    if (hashAuthContents(refreshedContents) === hashAuthContents(originalContents)) {
      return {
        name: profile.name,
        fileName: profile.fileName,
        email: profile.email || expectedEmail || null,
        backupFileName: null,
      };
    }
    const saved = saveAuthProfile(refreshedContents, {
      profileName: profile.name,
      expectedEmail,
      directory,
    });
    onStep(`Saved refreshed Codex auth profile ${saved.fileName}.`);
    return saved;
  } finally {
    fs.rmSync(tempHome, {
      recursive: true,
...
async function acquireAuthProfile(account, {
  ...
  const existingDestination = profiles.find(
    (profile) => profile.name.toLowerCase() === destinationName.toLowerCase(),
  );
  if (
    existingDestination
    && existingDestination.email?.toLowerCase() === email.toLowerCase()
    && existingDestination.valid
  ) {
    try {
      return await refreshExistingAuthProfile(existingDestination, {
        expectedEmail: email,
        directory,
        timeoutMs,
        spawnCodexProcess,
      ...
      onStep(
        `Saved session refresh failed; falling back to browser login: ${error.message}`,
      );
    }
  }
  const tempHome = createTempCodexHome();
